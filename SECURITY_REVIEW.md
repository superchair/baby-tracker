...
| Severity | Finding |
|---|---|
| **High** | **Dashboard fetches the entire 30-day event list just to read the first item** — `frontend/src/pages/Dashboard.tsx` calls `getEvents({type:'FEEDING'})` / `getEvents({type:'DIAPER'})` with no range, downloading 150–300+ full events (at current logging pace) on every dashboard load and after every single log action, when only the single most-recent record is needed. The efficient query pattern (`Limit: 1`) already exists in `infra/lambda/reminderCheck/index.ts` and just isn't reused here. |
